...
type PayoutAddressActionButtonProps = {
  completed: boolean;
  registered: boolean;
};

export function PayoutAddressActionButton({
  completed,
  registered,
}: PayoutAddressActionButtonProps) {
  ...
  async function claim() {
    setPending(true);
    setError(null);
    const res = await fetch("/api/missions/payout-address", { method: "POST" });
    if (!res.ok) {
      setPending(false);
      setError(
        res.status === 409
          ? "아직 출금 주소를 등록하지 않았거나 이미 보상을 받았습니다."
